...
@implementation NSDate (YYBInternetDateTime)
...
+ (void)getCurrentTimeFromService:(void (^)(NSString * currentTimeStr, NSError *error))completionHandler{
    ...
    NSURLSessionDataTask *task = [session dataTaskWithURL:[NSURL URLWithString:@"http://www.baidu.com"] completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error) {
            ...
            NSLog(@"%@",httpResponseHeaderFields[@"Date"]);
            
            
            
            NSDate *date = [NSDate dateFromRFC822String:httpResponseHeaderFields[@"Date"]];
            ...
            NSLog(@"%@",localeDate.description);
            if (completionHandler) {
                completionHandler([localeDate.description substringWithRange:NSMakeRange(0, localeDate.description.length-6)],nil);
            }
